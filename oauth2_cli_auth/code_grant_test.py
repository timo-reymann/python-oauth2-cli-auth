...
from oauth2_cli_auth.code_grant import refresh_access_token
...
def exchange_code_for_response(create_urlopen_mock):
    with create_urlopen_mock(io.BytesIO(b'{"access_token":"the_token","token_type":"Bearer","expires_in":3600,"refresh_token":"the_refresh_token","scope":"create"}')):
        response = exchange_code_for_response(client_info, "http://localhost:123", "code")
